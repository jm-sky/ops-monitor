...
class HealthComponent(BaseModel):
    ...
    reason: str | None = Field(
        None, description="Human-readable explanation, especially for degraded/failed."
    )
    since: str | None = Field(
        None, description="ISO 8601 timestamp of when this status began."
    )
    checked_at: str | None = Field(
        None, description="ISO 8601 timestamp of the last actual status verification."
    )
...
class HealthResponse(BaseModel):
    ...
    status: Literal["ok", "degraded", "failed"] = Field(
        ...,
        description=(
            "Overall application status. Must reflect the worst status across all components: "
            "failed > degraded > ok."
        ),
    )
    version: str | None = Field(None, description="Application/release version string.")
    environment: str | None = Field(
        None, description="Deployment environment (e.g. production, staging)."
    )
    components: dict[str, HealthComponent] | None = Field(
        None, description="Per-component statuses keyed by component name."
    )
    ...
    meta: dict[str, MetaValue] | None = Field(
        None,
        description=(
            "Arbitrary key-value parameters specific to this application "
            "(e.g. ksef_env, tenant, feature_flags). Values must be scalar."
        ),
    )
