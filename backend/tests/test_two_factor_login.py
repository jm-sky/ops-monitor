...
def mock_repository() -> AsyncMock:
    repo = AsyncMock(spec=TwoFactorRepositoryInterface)
    # verify_totp_login / complete_passkey_authentication both look up the
    # user's token_version via repo.db directly (not a repository method).
    db = AsyncMock()
...
class TestVerifyTotpLogin:
    ...
    async def test_successful_totp_login_mints_tfa_verified_tokens(self, two_factor_service: TwoFactorService) -> None:
        ...
        assert access_payload["jti"]  # session hardening kept intact
