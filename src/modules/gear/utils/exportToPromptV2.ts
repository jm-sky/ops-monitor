/**
 * Export container data to markdown format for AI prompts (V2 - Unified Model)
 */
...
interface ExportOptionsV2 {
  ...
  getItemById?: (id: string) => IGearItemV2 | undefined
...
function formatItem(
  ...
  if (options.showWeight !== false && item.weight) {
    const totalWeight = item.weight * (item.quantity ?? 1)
    const weightText = formatWeight(totalWeight, item.weightUnit ?? 'g', options.locale)
...
function formatNestedContainer(
  ...
  const typeLabel = options.getContainerTypeLabel
    ? options.getContainerTypeLabel(container.containerType || 'backpack')
    : (container.containerType || 'backpack')
  ...
  // Get container items using V2 method
  const children = options.getChildrenOfItem ? options.getChildrenOfItem(container.id) : []
...
function generateToBuySummary(
  ...
  const { t, getChildrenOfItem } = options
  ...
  containers.forEach(container => {
    const children = getChildrenOfItem ? getChildrenOfItem(container.id) : []
    const items = children.filter(child => child.itemType === 'item')
...
/**
 * Export single container to markdown (compact format) - V2
 */
export function exportContainerToPromptV2(
  container: IGearItemV2,
  options: ExportOptionsV2 = {},
): string {
  const {
  ...
  const typeLabel = getContainerTypeLabel
    ? getContainerTypeLabel(container.containerType || 'backpack')
    : (container.containerType || 'backpack')
  const containerId = generateContainerId(container.name)
  ...
  const nestedContainers: IGearItemV2[] = children.filter(child => child.itemType === 'container')
...
/**
 * Export multiple containers to markdown - V2
 */
export function exportContainersToPromptV2(
  containers: IGearItemV2[],
  options: ExportOptionsV2 = {},
): string {
  const { t } = options
  ...
  containers.forEach((container, index) => {
    ...
    const containerMarkdown = exportContainerToPromptV2(container, exportOptionsWithoutLegend)
