...
import ItemImageCardControls from './ItemImageCardControls.vue'
...
function handleDragLeave(event: DragEvent) {
  // Only emit if we're actually leaving the element (not just moving to a child)
...
<template>
  ...
  >
    ...
    <!-- Overlay controls (editable only) -->
    <ItemImageCardControls
      v-if="editable"
      :class="isDragging ? 'pointer-events-none' : ''"
      :image
